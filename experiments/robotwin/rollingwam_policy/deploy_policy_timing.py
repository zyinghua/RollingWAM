...
class WorldActionRobotWinPolicy:
    ...
    def _infer_action_chunk(self, observation: Dict[str, Any], instruction: str) -> np.ndarray:
        ...
        def run_inference(seed: Optional[int]):
            return self.model.rolling_act(
        ...
        timing_device = torch.device(self.model.device)
        if self.timing_enabled and not self._timing_warmed_up:
            rng_devices = [timing_device] if timing_device.type == "cuda" else []
            try:
                with torch.random.fork_rng(devices=rng_devices):
                    with torch.no_grad():
                        run_inference(self.seed if self.seed is not None else 0)
        ...
        if self.timing_enabled and timing_device.type == "cuda":
            torch.cuda.synchronize(timing_device)
        infer_t0 = time.perf_counter() if self.timing_enabled else 0.0
        with torch.no_grad():
            pred = run_inference(self.seed)
        if self.timing_enabled:
            if timing_device.type == "cuda":
                torch.cuda.synchronize(timing_device)
