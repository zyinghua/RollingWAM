...
def main(usr_args):
    ...
    if instruction_type not in {"seen", "unseen"}:
        raise ValueError(
            f"`instruction_type` must be 'seen' or 'unseen', got: {instruction_type!r}"
        )
    ...
    result_payload = {
        ...
        "instruction_type": instruction_type,
        "episodes": test_num,
        "success_rate": float(suc_num) / float(test_num),
        "reward": np.asarray(task_total_reward / test_num, dtype=float).reshape(-1).tolist(),
    }
    if len(result_payload["reward"]) == 1:
        result_payload["reward"] = result_payload["reward"][0]
    with open(save_dir / "result.json", "w", encoding="utf-8") as file:
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        try:
            ...
            # This seed passed expert validation but failed while rebuilding the
            # rollout environment. Do not count it as an evaluated episode.
            succ_seed -= 1
        ...
        if TASK_ENV.eval_video_path is not None:
            ...
            if current_video_path is None or not current_video_path.exists():
                raise FileNotFoundError(
                    f"Expected evaluation video file not found: {current_video_path}"
                )
            renamed_video_path = (
                Path(TASK_ENV.eval_video_path)
                / f"episode{episode_idx}_success-{str(succ).lower()}.mp4"
            )
