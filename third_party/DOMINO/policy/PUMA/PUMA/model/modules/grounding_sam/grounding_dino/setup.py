...
def get_extensions():
    ...
    if CUDA_HOME is not None and (torch.cuda.is_available() or "TORCH_CUDA_ARCH_LIST" in os.environ):
        ...
        # 支持 TORCH_CUDA_ARCH_LIST：已有 torch 环境时可用，如 "8.0;8.6;8.9"（8.9=RTX 4090）
        arch_list = os.environ.get("TORCH_CUDA_ARCH_LIST")
        if arch_list:
            for arch in arch_list.replace(" ", "").split(";"):
                arch = arch.strip()
                if arch:
                    # "8.0" -> compute_80, sm_80
                    sm = arch.replace(".", "")
                    nvcc_args.append("-gencode=arch=compute_{},code=sm_{}".format(sm, sm))
            print("Using TORCH_CUDA_ARCH_LIST: {}".format(arch_list))
        else:
            nvcc_args += [
        ...
        if not arch_list and cuda_version is not None and cuda_version >= 12.8:
            extra_compile_args["nvcc"].append("-gencode=arch=compute_120,code=sm_120")  # RTX 5090
        elif not arch_list and cuda_version is not None and cuda_version < 12.8:
