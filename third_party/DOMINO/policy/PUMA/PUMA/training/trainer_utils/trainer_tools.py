    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # filter out frozen or non-trainable parameters
            params = [
                p for p in module.parameters() if p.requires_grad and id(p) not in frozen_params
            ]
...
def _to_pil_image(image):
    """Normalize image input to a PIL.Image."""
    ...
    if isinstance(image, np.ndarray):
        # Support CHW tensors that may come from some preprocessing code.
        ...
        if image.dtype != np.uint8:
            if np.issubdtype(image.dtype, np.floating):
                ...
                if image.size and float(image.min()) >= 0.0 and float(image.max()) <= 1.0:
                    image = image * 255.0
        ...
        try:
            ...
            raise ValueError("Unsupported image type or structure.") from exc
...
def resize_images(images, target_size=(224, 224)):
    ...
    """
    if isinstance(images, list):  # if it is a list, recursively process each element
