...
def main(args):
    ...
    if use_wandb and WANDB_AVAILABLE:
        wandb.init(
            project="VLA",
            name=f"RoboTwin_ACT_{task_name}",
            config=config,
        )
    elif use_wandb and not WANDB_AVAILABLE:
        print("Warning: wandb is not available. Install it with 'pip install wandb'")
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if use_wandb:
            val_logs = {f"val/{k}": v.item() for k, v in epoch_summary.items()}
            val_logs["val/min_loss"] = min_val_loss
            val_logs["epoch"] = epoch
            wandb.log(val_logs, step=global_step)
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            if use_wandb:
                train_logs = {f"train/{k}": v.item() for k, v in forward_dict.items()}
                train_logs["lr"] = optimizer.param_groups[0]["lr"]
                wandb.log(train_logs, step=global_step)
