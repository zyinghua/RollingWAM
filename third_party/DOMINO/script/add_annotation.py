...
def get_method_source(filename, method_name):
    """
    Extract the source code of a specified method in a class.
    """
...
def replace_method_in_file_with_comments(filename, method_name, new_method_source):
    """
    Replace the method in the specified class with new content (preserving comments and original structure).
    """
    with open(filename, "r", encoding="utf-8") as f:
        lines = f.readlines()
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and item.name == method_name:
                    start_line = item.lineno - 1  # lineno is 1-based
                    end_line = _get_function_end_line(item, lines)

                    # Split new method content by lines, preserving indentation structure
                    # Note: new_method_source should be the original string from new.txt
                    new_lines = new_method_source.splitlines(keepends=True)

                    # Replace the corresponding line range
...
system_prompt = """
Role: You are a professional programmer with strong Python programming skills, able to quickly understand and modify code. You will add corresponding explanations to the code according to my requirements.
Task: I will give you a piece of Python code, and you need to add comments according to my requirements. Note that you cannot modify the code content in any way, you can only add comments.
- In play_once:
    - You need to understand the following available function calls:
        - I defined the self.move() function, which can take up to two parameters, namely actions_by_arm1 and actions_by_arm2.
        - The following functions return actions (i.e., an action sequence):
    ...
    - You need to comment on each self.move to explain what this action/combination of actions is doing. The comments here must **use English**.
...
Constraint: You must not modify the code content, only add comments.

Now, start your work! You only need to output the code block.
"""
