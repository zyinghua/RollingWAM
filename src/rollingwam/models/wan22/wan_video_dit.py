...
class WanVideoDiT(torch.nn.Module):
    ...
    def _validate_forward_inputs(
        ...
        if timestep.ndim == 2:
            if timestep.shape[1] != num_latent_frames:
                raise ValueError(
                    f"Per-frame `timestep` must be [B, T] with T={num_latent_frames}, got {tuple(timestep.shape)}"
                )
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            if timestep.ndim == 2:
                # Per-frame timesteps (rolling ramp): caller controls every frame, including
                # clean conditioning frames (t=0).
                token_timesteps = timestep.unsqueeze(2).expand(batch_size, x.shape[2], tokens_per_frame)
            else:
